...
import os, socket, select, signal, errno,  time, binascii, struct, hashlib
from winpty import PTY
from util.error import ConnectError
from util import fcntl
...
class L2tpClient(object):
            ...
            if plen > 12:
                ...
                while avp_offset < plen:
                    ...
                    if avp_type == 0:  # control message
                        (avp_control_msg,) = struct.unpack('!H', bytes(avp_values, encoding='utf-8'))
                        if self.debug:
                        ...
                        (avp_protocol_version, avp_protocol_revision) = struct.unpack('!BB', bytes(avp_values,
                                                                                                   encoding='utf-8'))
                        if self.debug:
                            print("<< avp protocol version : %d, revision %d" % (
                        ...
                        if self.debug:
                            print("<< avp framing capabilities, async : %s, sync %s" % (
                                avp_framing_async, avp_framing_sync))
                        ...
                        if self.debug:
                            print("<< avp bearer capabilities, analog : %s, digital %s" % (
                                avp_bearer_analog, avp_bearer_digital))
                    elif avp_type == 6:  # firmware revision
                        (avp_firmware_rev,) = struct.unpack('!H', bytes(avp_values, encoding='utf-8'))
                        if self.debug:
                        ...
                        (avp_tunnel_id,) = struct.unpack('!H', bytes(avp_values, encoding='utf-8'))
                        self.tunnel_id = avp_tunnel_id
                        if self.debug:
                            print("<< avp tunnel id : %d" % avp_tunnel_id)
                    elif avp_type == 10:  # receive window size
                        (avp_receive_wsize,) = struct.unpack('!H', bytes(avp_values, encoding='utf-8'))
                        if self.debug:
                        ...
                        (avp_session,) = struct.unpack('!H', bytes(avp_values, encoding='utf-8'))
                ...
                if ack_ns == self.ns:  # ack packet
                    if self.last_sent_packet_type == 1:  # SCCRQ
                        ...
                        print("<! Ack Packet Type %d" % self.last_sent_packet_type)
    ...
    def make_sccrq(self):
        ...
        l2tp_data = header_data + bytes(avp_data, encoding='utf-8')
        return l2tp_data
